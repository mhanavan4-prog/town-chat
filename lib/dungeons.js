...
module.exports = function createDungeons({ players, send, isEvading, absorbIncomingDamage, noteAttacked, DUNGEON_ROOMS, DUNGEON_SIZE }) {
  ...
  // 🩸 Boss enrage: the signature bosses fight harder as you wear them down —
  // below half their (party-scaled) health their blows land ~40% heavier, so
  // the marquee fights escalate toward a real finish. Rank-and-file mobs are
  // unaffected.
  const BOSS_ENRAGE_THRESHOLD = 0.5;
  const BOSS_ENRAGE_MULT = 1.4;
  function bossEnrageMult(m, preset) {
    if (!m || !m.boss) return 1;
    const maxH = m.scaledMax || (preset && preset.maxHealth) || 1;
    return m.health <= maxH * BOSS_ENRAGE_THRESHOLD ? BOSS_ENRAGE_MULT : 1;
  }
    ...
    for (const m of dungeonMobs) {
      ...
      if (m.scaredUntil > now && nearestP) {
        ...
        if (dist < preset.strikeRange && (!m.lastHitAt || now - m.lastHitAt >= preset.hitCooldownMs)) {
          m.lastHitAt = now;
          const baseDmg = Math.round((preset.dmgMin + Math.floor(Math.random() * (preset.dmgMax - preset.dmgMin + 1))) * bossEnrageMult(m, preset));
          const dmg = absorbIncomingDamage(nearestP, baseDmg);
  ...
  return { dungeonTierForLevel, dungeonMobs, dungeonMobMaxHealth, nearestDungeonPlayer, resetDungeonRoom, tickDungeon, bossEngagedScale, bossEnrageMult, playersInRoom, DUNGEON_ENTRY_BY_TIER, DUNGEON_BOSS_RESPAWN_MS, PARTY_BOSS_HP_PER_ALLY };
};
